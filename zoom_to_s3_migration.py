...
def upload_one(token, base_prefix, rec_file):
    """Download one file from Zoom and put it in S3.

    Returns (status, note) where status is:
        "uploaded"  -- the object is confirmed present in S3
        "skipped"   -- a file type this pipeline does not store
        "failed"    -- did not land

    WHY THREE STATES AND NOT A BOOLEAN
      The first version returned True for a skip. That meant a meeting whose
      files were ALL skipped produced no errors, and so was logged as
      "uploaded and verified" -- with nothing uploaded at all. If
      --cleanup-zoom had been on, the Zoom copy would then have been deleted.

      Skipped and uploaded must be distinguishable, because "no errors" is not
      the same as "something landed".

    VERIFICATION IS UNCONDITIONAL
      It used to run only `if expected:` -- so a file Zoom reported as 0 bytes,
      or with no file_size at all, was never checked. Now head_object is always
      called: the object must EXIST, and its size must match when Zoom told us
      one. Existence is the part that actually matters.
    """
    ftype = (rec_file.get("file_type") or "").upper()
    folder = FILE_TYPE_FOLDER.get(ftype)
    if not folder:
        return "skipped", f"file_type={ftype or 'EMPTY'} not stored by this pipeline"

    ext = (rec_file.get("file_extension") or ftype).lower()
    key = f"{base_prefix}{folder}/{rec_file.get('id')}.{ext}"

    if s3_object_exists(key):
        return "uploaded", "already present"

    url = rec_file.get("download_url")
    if not url:
        return "failed", f"{ftype}: no download_url"

    try:
        with requests.get(url, headers={"Authorization": f"Bearer {token}"},
                          stream=True, timeout=900) as resp:
            if resp.status_code != 200:
                return "failed", f"{ftype}: download HTTP {resp.status_code}"
            s3.upload_fileobj(resp.raw, S3_BUCKET, key)
    except Exception as exc:
        return "failed", f"{ftype}: transfer failed: {exc}"

    # Always verify. Existence first -- that is what was missed before.
    try:
        got = s3.head_object(Bucket=S3_BUCKET, Key=key)["ContentLength"]
    except Exception as exc:
        return "failed", f"{ftype}: uploaded but NOT FOUND in S3 afterwards: {exc}"

    expected = rec_file.get("file_size")
    if expected and got != expected:
        return "failed", f"{ftype}: size mismatch: zoom {expected}, s3 {got}"
...
def main():
    ...
    for idx, e in enumerate(todo, 1):
        ...
        # A meeting where nothing actually landed must NOT count as migrated --
        # and must never be queued for Zoom deletion.
        if not errs and n_uploaded == 0:
            errs.append(f"nothing uploaded: all {n_skipped} file(s) were of "
                        f"types this pipeline does not store")
        ...
        if errs:
            ...
            log(f"        {n_uploaded} file(s) uploaded and verified"
            ...
            if args.cleanup_zoom:
                ...
                try:
                    proc.enqueue_delete_job(
                        ...
                        # MUST be a value the cleaner recognises as "everything
                        # has arrived". It waits for a transcript before
                        # deleting, and a value it does not know is treated as
                        # "still waiting" -- so the message is retried for two
                        # hours and the Zoom copy is never removed.
                        #
                        # For a migrated recording the transcript is ALREADY in
                        # S3 (we uploaded it and byte-verified it), so the
                        # transcript-completed state is exactly what is true.
